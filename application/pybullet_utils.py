...
def pairwise_collision(body1, body2, max_distance=MAX_DISTANCE):
    return p.getContactPoints(body1, body2) != ()


def pairwise_link_collision(body1, link1, body2, link2, max_distance=MAX_DISTANCE):
...
def dump_body(body):
    ...
    print(
        "Link id: {} | Name: {} | Mass: {}".format(
            -1, get_base_name(body), get_mass(body)
        )
    )
    for link in get_links(body):
        print(
            "Link id: {} | Name: {} | Parent: {} | Mass: {}".format(
                link,
...
def inverse_kinematics(body, eef_link, position, orientation=None):
    if orientation is None:
        jv = p.calculateInverseKinematics(
            bodyUniqueId=body,
...
def create_frame_marker(
    ...
    pts = np.array(
        [[0, 0, 0], [line_length, 0, 0], [0, line_length, 0], [0, 0, line_length]]
    )
    ...
    if replace_frame_id is not None:
        x_id = p.addUserDebugLine(
            ...
            line_width,
            life_time,
            replaceItemUniqueId=replace_frame_id[0],
        )
        y_id = p.addUserDebugLine(
            ...
            line_width,
            life_time,
            replaceItemUniqueId=replace_frame_id[1],
        )
...
def create_arrow_marker(
    ...
    Add a dot towards the positive direction.
    """
    ...
    color = (
        ...
        else rgb_colors_1[color_index % len(rgb_colors_1)]
    )
...
def plot_heatmap_bar(cmap_name, vmin=-1, vmax=-1):
    ...
    cb = colorbar.ColorbarBase(
        plt.gca(), cmap=cmap, norm=norm, orientation="horizontal"
    )
...
rgb_colors_1 = np.array(rgb_colors_255) / 255.0
